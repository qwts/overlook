...
| Reorder ordinary albums from an expanded-rail handle, keyboard grab model, Option+Up/Down, or shared menu commands; collapsed mode retains menu commands; order persists atomically, refreshes the manifest for empty albums, and is undoable without changing active source, selection, membership, photo drops, or protected albums                                                                               | ✅ #225 (PR #674)                        | `tests/e2e/album-reorder.spec.ts` + `Sidebar.stories.tsx` + persistence/history/payload unit tests — ledger id `m10-album-reorder`                                |
