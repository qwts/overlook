...
validated once through the normal ready and merge-queue lifecycle. Version-cut
waits for the exact main-push `CI` gate and dispatches no equivalent suite before
tagging `v0.x.y`.
...
gate. Generic CI is reused; release-specific macOS and Windows builds, packaging,
