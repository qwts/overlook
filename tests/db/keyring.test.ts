...
describe('keyring registry (#517, migration 36)', () => {
  test('migration 36 remains registered, mints a reference for legacy rows, and enforces the registry constraints', () => {
    assert.deepEqual(
      MIGRATIONS.filter((migration) => migration.version === 36).map((migration) => ({ version: migration.version, name: migration.name })),
