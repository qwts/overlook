...
describe('photo edit service (#493)', () => {
  ...
  test('an offloaded original defers the bake; a failed bake reports failure but keeps the head', async () => {
    ...
    assert.equal(new EditBakeDebtRepository(deferred.db).pending('P1'), deferredResult.head?.id);
    ...
    assert.equal(new EditBakeDebtRepository(failing.db).pending('P1'), failedResult.head?.id);
