...
describe('embedding execution-provider fallback', () => {
  ...
  test('ordinary restoration and derivative repair reactivate deferred candidates', () => {
    ...
    assert.match(
      index,
      /libraryChanged: \(photoIds, membership, albumIds\) => \{\s+applicationEvents\.libraryChanged\(\{\s+photoIds: \[\.\.\.photoIds\],\s+membership,\s+\.\.\.\(albumIds === undefined \? \{\} : \{ albumIds: \[\.\.\.albumIds\] \}\),\s+\}\);\s+notifyEmbeddingEligibilityChanged\(photoIds\);/u,
    );
    assert.match(
      index,
      /ordinaryChanged: \(photoIds\) => \{\s+applicationEvents\.libraryChanged\(\{ photoIds: \[\.\.\.photoIds\], membership: 'library' \}\);\s+notifyEmbeddingEligibilityChanged\(photoIds\);/u,
    );
