...
const OVERRIDDEN_COLUMNS: Readonly<Record<string, string>> = {
  ...
  preview_missing: "CASE WHEN file_kind IN ('jpeg', 'png', 'raw', 'heic', 'gif', 'webp') THEN 1 ELSE 0 END",
...
export class VariantRepository {
  ...
  family(contentHash: string): VariantFamily {
    const rows = queryAll<PhotoRow>(
      this.db,
      `SELECT p.*, l.status AS sync_state, l.coverage AS coverage, ${PHOTO_KEY_PRESENT_SQL} AS key_present, ${PHOTO_MISSING_KEY_SQL} AS missing_key_id, p.imported_at AS sort_key
