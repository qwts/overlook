...
export function select(order: keyof typeof ORDERINGS): string {
  return `
  SELECT p.*, l.status AS sync_state, l.coverage AS coverage, ${PHOTO_KEY_PRESENT_SQL} AS key_present, ${ORDERINGS[order].expr} AS sort_key
...
export function selectRanked(): string {
  return `
  SELECT p.*, l.status AS sync_state, l.coverage AS coverage, ${PHOTO_KEY_PRESENT_SQL} AS key_present, photos_fts.rank AS sort_key
  FROM photos_fts
