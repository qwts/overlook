...
function* probeCandidates(db: BetterSqlite3.Database, keyId: number) {
  // Current owners are most likely to authenticate. Retained references can
  // outlive regenerated bytes, so exhaust that tier without buffering it all.
  for (const direct of [1, 0]) {
    let after = '';
    for (;;) {
      const rows = queryAll<{ id: string; content_hash: string; derivative_key: string; assetOwnerId: string | null }>(
        db,
        `SELECT id, content_hash, derivative_key, asset_owner_id AS assetOwnerId FROM photos p
         WHERE id > @after AND (p.key_id = @keyId) = @direct
           AND (p.key_id = @keyId OR EXISTS (
             SELECT 1 FROM retained_photo_keys r WHERE r.photo_id = p.id AND r.key_id = @keyId))
         ORDER BY id LIMIT @limit`,
        { keyId, direct, after, limit: PROBE_PAGE_SIZE },
      );
      for (const row of rows) {
        after = row.id;
        yield row;
      }
      if (rows.length < PROBE_PAGE_SIZE) break;
    }
  }
}
...
export async function probeKeyAgainstStore(
  ...
  for (const row of probeCandidates(db, keyId)) {
    const openers = [
