 ...
 * organizational tags (schema 9, #505), Smart Albums (schema 10, #514), edit
 * revisions (schema 11, #493), and provenance evidence (schema 12, #495).
 * One place so `main/index.ts` stays a wiring file. */
export function libraryDataBackupDeps(
  ...
  | 'activitySnapshot'
  | 'boardsSnapshot'
  | 'galleryPolicySnapshot'
  | 'hiddenAlbumIdsSnapshot'
  | 'albumTreeSnapshot'
